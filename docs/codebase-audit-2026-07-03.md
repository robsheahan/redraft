...
> **R3 — BUILT 2026-07-03.** `lib/skill-prompt.ts` (`formatStudentSkillProfile`, `formatCohortSkillMatrix`) injects the measured skill data into: the longitudinal student profile (`lib/student-profile.ts`), the four student cards, and the whole-cohort pattern cards `common_gaps`/`things_done_well` (`api/insights-card-generate.ts`). Guardrails baked into the block text (developmental, never a mark/band; anchor, don't contradict). Best-effort — a skill read failure omits the block. Formatters unit-verified.
...
> **R6 — BUILT 2026-07-03.** Provenance labels on every insight card face (`provenanceFooter` helper + thin-evidence hedge on the Skill breakdown; within-student-method notes on growth/trajectory/movement; task count added to the LLM cohort + student card headers). Backend: the `common_gaps` cohort prompt tags each entry "full feedback" (Sonnet) vs "brief auto-signal" (Haiku) with a weighting instruction. The recency-sort/fingerprint half of R6 shipped earlier in the audit fixes. UI verified headless; the prompt tagging can only be validated with live LLM runs.
