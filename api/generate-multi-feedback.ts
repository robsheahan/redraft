...
/** Map a client `answers` array to { question_id -> text }, trimmed. */
function sanitiseAnswers(input: any): Map<string, string> {
...
export default withHandler({ methods: ['POST'], label: 'generate-multi-feedback' }, async (req, res, ctx) => {
  ...
  const hasContent = (qid: string) => !!(submitted.get(qid) || '').trim();
  if (!questions.some((q) => hasContent(q.id))) {
    return res.status(400).json({ error: 'Write at least one answer before getting feedback.' });
  }
  const totalChars = questions.reduce((n, q) => n + (submitted.get(q.id) || '').length, 0);
