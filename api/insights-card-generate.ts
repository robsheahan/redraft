...
const KIND_CONFIG: Record<string, {
  ...
  bottom_decile: {
    ...
    buildSystemPrompt: (schoolName, sample) => [
      ...
      ``,
      `You'll receive each student's AI improvement feedback. Identify UP TO 3 dominant patterns of mistakes — the things appearing across multiple students that a head of teaching & learning should action. Name only as many as the sample genuinely supports (1–3); do not pad to three.`,
      ``,
      `RULES:`,
      ...
      `- Entries tagged "full feedback" (Sonnet) carry richer evidence than "brief auto-signal" (Haiku, from exam tasks); weight the fuller feedback more heavily when they conflict.`,
      `- prevalence_note describes the REVIEWED SAMPLE only — you saw a slice, not the whole cohort. Never claim a cohort-wide fraction.`,
    ].join('\n'),
  ...
  top_decile: {
    ...
    buildSystemPrompt: (schoolName, sample) => [
      ...
      ``,
      `These students are already performing well. You'll receive each student's AI improvement feedback. Identify UP TO 3 highest-impact next steps that would stretch them further — the things that would move "high" responses to "exceptional". Only as many as the sample supports (1–3); do not pad to three.`,
      ``,
      `RULES:`,
      `- Frame next steps as STRETCH goals, not remedial fixes.`,
      `- Be specific about what the upgrade looks like in their writing.`,
      `- No mark or band predictions.`,
      `- Entries tagged "full feedback" (Sonnet) carry richer evidence than "brief auto-signal" (Haiku); weight the fuller feedback more heavily.`,
    ].join('\n'),
...
function buildImprovementsPrompt(rows: any[], label: string): string {
  const lines = rows.map((r, i) => {
    ...
    return [
      `--- ${label} #${i + 1} (${feedbackSourceLabel(r.task_mode)}) ---`,
