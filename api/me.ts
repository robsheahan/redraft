...
async function returnTaskDrafts(req: VercelRequest, res: VercelResponse, userId: string) {
  ...
  const { data, error } = await supabase
    .from('submissions')
    .select('id, draft_text, feedback, draft_version, created_at, question, course, criterion_marks, total_mark, teacher_comment, teacher_annotations, graded_at, graded_by, submitted_for_marking, working_lines, part_working, input_mode, student_attachments, answers, question_marks')
    .eq('student_id', userId)
    .eq('task_id', taskId)
