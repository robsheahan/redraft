...
> **Untrusted student text shares prompts with authoritative content** (marking guidelines, teacher notes) with no "treat as data" hardening anywhere, and the model outputs it influences (skill_assessment) feed a **permanent** store (`student_skill_profile`) that drives Lesson Differentiator differentiation and teacher insights.
