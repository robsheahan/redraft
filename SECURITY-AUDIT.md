...
| P8 | Medium | **No idempotency on submission insert** — draft_version is read-then-insert with no unique constraint; a double-click = two drafts + 6 Sonnet calls (`generate-feedback.ts:219-241,470`; `submit-for-marking.ts:93-97`). | Unique index `(student_id, task_id, draft_version)` (+ own_task variant) + conflict handling. |
