...
- **Readers** — the **Lesson Differentiator** queries `student_skill_profile` via `readSkillProfile()`, and the **cohort skill matrix** insight card (see Insights → Skill breakdown) aggregates it via `computeSkillMatrix()` in `lib/skill-matrix.ts`. (The LLM profile/insight syntheses do NOT yet read the skill store — that's insights roadmap R3.)
