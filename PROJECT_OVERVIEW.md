...
- **Readers** — the **Lesson Differentiator** queries `student_skill_profile` via `readSkillProfile()`; the **cohort skill matrix** card aggregates the snapshot via `computeSkillMatrix()` in `lib/skill-matrix.ts`; and the **cohort skill growth** card aggregates the observation history via `computeSkillGrowth()` in `lib/skill-history.ts` (see Insights → Skill breakdown / Skill growth). (The LLM profile/insight syntheses do NOT yet read the skill store — that's insights roadmap R3.)
...
- `skill-history.ts` — `computeSkillGrowth()`, the deterministic cohort skill-growth card (within-student deltas over time from `skill_observations`)
