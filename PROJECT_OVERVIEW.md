...
- **`submit-for-marking.ts`** — POST. Students submit a final draft without generating AI feedback. Saves a submission row with `submitted_for_marking = true` (no Anthropic call), locks the task for the student (same lock semantics as a graded submission). Rejects if the student already has a graded or submitted-for-marking submission. Clears the autosave row.
...
- **`submit-for-marking-migration.sql`** — Adds `submissions.submitted_for_marking` boolean + partial index on `(task_id, student_id) where submitted_for_marking = true`.
