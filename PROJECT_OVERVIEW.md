...
- **Readers** — the profile narrative, insights, and the **Lesson Differentiator** (now built — see its section) query `student_skill_profile` via `readSkillProfile()` in `lib/skill-profile.ts`.
